...
export default async function SupportPage() {
  ...
  return (
    <div dir="rtl" className="space-y-8">
      <h1 className="text-2xl font-semibold text-navy">{supportAr.centerTitle}</h1>

      <section className="space-y-4">
        <h2 className="text-lg font-medium text-navy">{supportAr.newTicketTitle}</h2>
      ...
      <section className="space-y-4">
        <h2 className="text-lg font-medium text-navy">{supportAr.myTicketsTitle}</h2>
        {tickets.length === 0 ? (
          <p className="text-muted">{supportAr.noTicketsClient}</p>
        ) : (
