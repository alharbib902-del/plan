...
| **D21** | **Idempotent cashback award**: `award_cashback_for_booking` RPC checks `EXISTS (earn for booking_id)` BEFORE INSERT, returns `{ok:true, already_awarded:true, skipped_reason:'duplicate_earn_for_booking'}` if found. DB-side `UNIQUE INDEX (booking_id) WHERE event_type='earn'` enforces the invariant defense-in-depth. | Prevents double-award across all v1 code paths: trigger replay on same `paid` UPDATE, manual admin re-invocation of the RPC, and concurrent races between two transactions both holding the booking row briefly. Round 8 PR #80 F18 fix — earlier wording mentioned `refunded→paid re-confirmation`, but D24+F14 make that transition impossible in v1 (payment_status is immutable post-paid). Phase 13.1 refund RPC will introduce the only sanctioned re-entry path, and D21 will be the safety net there too. |
...
| **D23** | **Effective paid amount is computed inline** as `amount_paid = total_amount - cashback_redemption_sar` at award-time. There is NO stored `amount_paid_sar` column (Round 1 PR #80 F2 verified the ghost; Round 8 PR #80 F21 reworded D23 title to stop suggesting one exists). `award_cashback_for_booking` reads `total_amount` + `cashback_redemption_sar` from `bookings` inside the FOR UPDATE lock and derives the value locally as `v_amount_paid`. The RPC return envelope uses the JSON key `amount_paid_sar` for the value (API naming convention, not a column reference). | Avoids stored-derived column inconsistency; the single source of truth is `(total_amount, cashback_redemption_sar)`. |
...
| **D29** | **Booking cancellation interaction** (Round 8 PR #80 F19 rewrite — clean source-of-truth statement, no inline contradictions). Three cases:  **(a) Pre-paid redemption allowed**: `redeem_cashback_for_booking` accepts bookings where `paid_at IS NULL` (per D7+F15). Client locks in cashback discount BEFORE the booking is paid; this debits balance + sets `cashback_redemption_sar`. **(b) Cancel BEFORE paid**: no cashback earned (trigger never fires). If a pre-paid redeem event was already posted, it STAYS in the ledger — the cashback was debited but no actual flight was delivered. Admin manually invokes `admin_adjust_cashback` (Phase 13.x) to restore balance when the cancellation was operator-driven; client-driven cancellations forfeit the redeem in v1 (Phase 13.1 may add `cancel_unused_redemption` RPC). **(c) Cancel AFTER paid**: blocked absolutely by D24 immutability (post-paid mutation = refund flow, deferred to Phase 13.1 per D25). | Avoids an auto-reversal layer that would be hard to test in v1; admin-driven correction is acceptable for low-volume v1 launch. Round 4 PR #80 F39 fix + Round 8 PR #80 F19 reword. |
...
CREATE TABLE IF NOT EXISTS client_loyalty_ledger (
  ...
  -- Invariants (Round 8 PR #80 F20 cleanup — historical refund_back
  -- fix-citations moved to D25; DDL block now reads cleanly without
  -- references to ENUM values that don't exist in v1).
  ...
  -- D21+F28: earn + redeem both require a booking_id. (See D25
  -- in §2 for refund-flow deferral context.)
