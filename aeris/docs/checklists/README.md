...
| [admin-inbox-smoke-test.md](admin-inbox-smoke-test.md) | `/admin/login` + `/admin/leads` end-to-end UI flow + cookie-tamper UI check. | Before every deploy that touches `lib/admin/*`, `lib/supabase/queries/leads.ts`, `app/(admin)/**`, or `components/admin/**`. Also weekly. |
