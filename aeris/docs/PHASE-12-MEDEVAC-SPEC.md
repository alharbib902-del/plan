...
| **D8** | `medevac_requests.patient_name_snapshot` + `patient_age_snapshot` are admin-only displays. Client portal + operator portal render the MEV-XXXX number + service level + severity ONLY — patient name redacted. | PII minimization. The patient name is required for dispatch coordination (operator needs to know who they're transporting) but only the admin needs the full name in UI; operators get it on the dispatch notification email/wa.me message (after they ACCEPT the offer). |
...
| **D10** | SLA response windows by severity: `critical=1h`, `moderate=4h`, `stable=24h`. Stored in `medevac_severity_sla` lookup table (§3.6) so admin can tune without code deploy. Cron checks `medevac_requests WHERE status='dispatched' AND dispatched_at + sla_interval < NOW()` → auto-escalate to admin via `founder_critical_escalation_email`. | Operator must quote within the SLA or auto-escalate. Critical=1h is the existing industry standard; stable=24h gives buffer for non-urgent transfers. |
| **D11** | Medical cert expiry cron: `*/30 * * * *` checks `aircraft_medical_certifications WHERE certification_expires_at <= NOW() + INTERVAL '30 days'` → flips `supports_*` to `false` AND emails the operator at 30/14/7/1 day(s) ahead via `expired_medical_cert_alert`. Past-expiry rows are excluded from distribution. | Defensive — prevents dispatching to expired certs even if operator forgets to renew. 30/14/7/1 day cascade gives operator time to renew before they lose dispatchability. |
...
CREATE TABLE IF NOT EXISTS medevac_requests (
  ...
  -- Covered status invariant (D13)
  CONSTRAINT medevac_requests_covered_invariant_check CHECK (
    (is_covered = false) OR (is_covered = true AND status = 'covered')
  ),
...
CREATE INDEX IF NOT EXISTS idx_medevac_requests_sla_pending
  ...
  WHERE status = 'pending' AND sla_escalated_at IS NULL;
...
Per-aircraft cert matrix (D7). Mirrors
...
CREATE TABLE IF NOT EXISTS aircraft_medical_certifications (
  ...
  CONSTRAINT aircraft_medical_certifications_expiry_future_check CHECK (
    certification_expires_at > created_at_placeholder()
  )
);
-- Note: created_at_placeholder() above is a sentinel — actual
-- CHECK in the migration uses `certification_expires_at > NOW()`
-- inside a BEFORE INSERT trigger so the check can fire on insert
-- but tolerate cron-flips (when cert expires + we down-flip the
-- supports_* BOOLs, the row stays with past-expiry timestamp).
-- See §4.7 — `expire_medical_certifications` cron.
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE medevac_requests
      ...
      REFERENCES medevac_subscriptions(id) ON DELETE SET NULL;
  END IF;
END $$;
```
...
CREATE TABLE IF NOT EXISTS aeris_shield_config (
  ...
  default_operator_id UUID REFERENCES operators(id) ON DELETE SET NULL,
...
NEW — no Phase 11 equivalent. Atomically:
1. Locks the subscription FOR UPDATE
2. Verifies `status='active'` + `end_date > NOW()` + (covered_events=-1 OR used_events < covered_events_at_signup)
3. Verifies the requested service_level is ≤ subscription's
   service_level_at_signup (e.g. individual plan can't claim CCT)
4. Increments `used_events`
5. Inserts the medevac_request with `is_covered=true`,
   `subscription_id=p_subscription_id`, `status='covered'`
6. Inserts a booking with `source_offer_table=NULL`,
   `source_offer_id=NULL` (covered bookings have no offer),
   `operator_id=aeris_shield_config.default_operator_id`
7. Audit log entry (PII redacted per D12)
...
### PR 1 — Backend + public form + admin intake (~2000 lines)

**Migration** `20260525000040_phase_12_pr_1_medevac_intake.sql`:
- Phase 1 prototype cleanup (§3.1 DO block)
- 7 ENUMs + 4 tables (medevac_requests, medevac_offers,
  medevac_severity_sla seed, aeris_shield_config seed,
  medevac_email_alert_status, plan_terms seed)
- 3 RPCs: §4.1, §4.2, §4.3 + §4.7 + §4.8

**Server Actions** (`app/actions/medevac-public.ts` +
`medevac-admin.ts`):
- `submitMedevacRequestPublic` — wraps §4.1
- `upsertMedicalCertification` — admin, mirrors
  `upsertCargoAircraftCapability` shape

**Pages:**
- `/medevac` — public intake form (stable only)
- `/admin/medevac` — queue (mirror /admin/cargo)
- `/admin/medevac/[id]` — detail (admin-only patient_name visible)
- `/admin/medevac/medical-certifications` — per-aircraft matrix

**Tests** (`lib/medevac/__tests__/`):
- `medevac-request-validators.test.ts` (~20 cases mirror
  cargo-request-validators)

### PR 2 — Authed portal + offer/booking + subscriptions (~1500 lines)

**Migration** `20260526000041_phase_12_pr_2_medevac_offers_subs.sql`:
- §4.4 + §4.5 + §4.6 + §4.9 RPCs
- Schema deltas (decline_reason + withdraw_reason on
  medevac_offers if not already in PR 1)

**Server Actions** (`medevac-clients.ts` + `medevac-operators.ts`):
- 4 client wrappers (submit authed, accept, decline, cancel,
  + subscribe to shield)
- 2 operator wrappers (submit_offer, withdraw_offer) +
  password_must_change guard
- 3 admin extensions (accept/decline/cancel on behalf +
  admin_activate_subscription)

**Pages:**
- `/me/medevac` — list client requests
- `/me/medevac/new` — authed form (all severities)
- `/me/medevac/[id]` — detail + offer list + accept/decline + cancel
- `/me/medevac/shield/subscribe` — plan picker + members form
- `/me/medevac/shield/[id]` — subscription detail + cancel auto-renew
...
- `/admin/medevac/[id]` — extended with accept/decline/cancel buttons
- `/admin/medevac/subscriptions` — admin queue of pending_payment subs
- `/admin/medevac/subscriptions/[id]/activate` — admin activation
- `/admin/medevac/shield-config` — set default_operator_id (D14)

**Tests:**
- `medevac-accept-flow.test.ts` (~10 cases mirror cargo)
- `medevac-booking-shape.test.ts` (~10 cases — pins BOTH
  non-covered AND covered booking shapes)
- `medevac-offer-validators.test.ts` (~20 cases)
- `aeris-shield-coverage.test.ts` (NEW — 8 cases pinning
  subscription event consumption rules)

### PR 3 — Distribution + SLA cron + notifications + canary (~1200 lines)

**Migration** `20260527000042_phase_12_pr_3_medevac_distribution.sql`:
- `medevac_dispatch_events_outbox` table (mirror cargo)
- `publish_medevac_dispatch_event` + `claim_medevac_dispatch_events`
  + `medevac_operator_last_dispatch_map` RPCs
- `medevac_requests` AFTER INSERT trigger (only for non-covered
  status='pending')
- `medevac_requests.founder_sla_escalated_at` column for
  per-request escalation throttle

**TS pipeline** (`lib/medevac/`):
- `scoring.ts` — pure scoring (capability + recency + rating +
  SLA-priority weighting for critical severity)
- `distribution.ts` — DB-backed dispatch
- `notifications.ts` — operator email + wa.me link (with
  patient name redacted per D8 — operator sees MEV-XXXX only
  until they accept)
- `founder-sla-escalation-email.ts` — Decision D10 escalation
  alert when cron detects past-SLA

**Cron routes:**
- `/api/cron/medevac/dispatch-drain` — every 5 minutes
  (faster than cargo because medevac SLAs are tighter)
- `/api/cron/medevac/sla-escalation` — every 5 minutes;
  scans `medevac_requests WHERE status='dispatched' AND
  dispatched_at + sla_interval < NOW() AND sla_escalated_at IS NULL`
- `/api/cron/medevac/expire-certifications` — every 30 min;
  flips `aircraft_medical_certifications.supports_*` to false
  when `certification_expires_at <= NOW()`; emails operator at
  30/14/7/1 day(s) ahead (D11)

**Admin extensions:**
- 7th `<ChannelHealth>` card on `/admin/operators/canary`
- `/admin/medevac/[id]/distribute` — manual dispatch (mirror
  cargo manual dispatch)

**Tests:**
- `medevac-distribution-scoring.test.ts` (~12 cases — adds
  SLA-priority weighting tests on top of cargo's scoring tests)
- `medevac-sla-escalation.test.ts` (~6 cases — pin escalation
  trigger logic)
- `medevac-outbox-drain.test.ts` (~6 cases mirror cargo)
- `medevac-cron-auth.test.ts` (5 cases mirror cargo)
- `medical-cert-expiry.test.ts` (~5 cases — pin 30/14/7/1
  day notification cascade)

---

## §6 Founder probes

5 probes for Phase 12 (probes 33-37; numbering continues from
Phase 11's probe 32).
...
1. Insert medevac request with `severity='critical'` +
   `dispatched_at = NOW() - INTERVAL '2 hours'` (past 1h SLA).
